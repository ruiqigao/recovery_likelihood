...
class Trainer:
  ...
  def train(self, output_dir, output_dir_ckpt, output_dir_thread, strategy):
    ...
    if FLAGS.eval:
      ...
      x_neg_seq, is_accepted = self.diffusion_ema.distribute_p_sample_progressive(noise)
    ...
    while i_iter < (self.hps.n_iters + 1):
      loss, grads_mean, grads_max, disp_ts, loss_ts, f_ts, is_accepted = self.distributed_train_fn_multisteps(dist_iter)
      ...
      if i_iter % 5000 == 0:
        ...
        noise = self.get_dist_tensor(noise)
        x_neg_seq = self.diffusion_ema.distribute_p_sample_progressive(noise, self.strategy)[0][:, :64]
  ...
  def eval_fid_is(self, full=False):
    ...
    for k in range(num_batch):
      ...
      noise = self.get_dist_tensor(noise)
      x_neg, _ = self.diffusion_ema.distribute_p_sample_progressive(noise, self.strategy)
